...
copy ${build_folder}/tmp/work/corei7-64-pxc-linux/fw/1.1.0-r0/image/etc/plcnext/fwversion ${output_folder}/version_info/
copy ${build_folder}/tmp/work/corei7-64-pxc-linux/fw/1.1.0-r0/image/etc/plcnext/FwVersionJenkins.properties ${output_folder}/version_info/
...
if [ "${SPECIAL_BUILD}" == "true" ]; then
    ...
    tar -czvf ${output_folder}/sources.tar.gz ${deploy_folder}/sources
fi

mkdir ${build_folder}"/temp"
cd ${build_folder}"/temp"
tar -xzf ${deploy_folder}/images/${MACHINE}/${DEVICE}-image-base-${MACHINE}.tar.gz
cd ${build_folder}"/temp/usr/share/"
tar -czf ${output_folder}/licenses.tar.gz common-licenses/
cd ${build_folder}
rm -rf temp/
